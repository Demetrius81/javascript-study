...
Vue.component("to-do-list", {
  ...
  methods: {
    ...
    remove(id) {
      let index = this.affairs.indexOf(this.affairs.find((x) => x.id === id));
  ...
  template: `
            <div>
                ...
                <ul>
                    <li v-for="item in affairs" :key="item.id">
                        {{item.name}}
...
Vue.component("card-box", {
  ...
  template: `
    <article class="catalog__shop">
                    <div>
                        <button @click="sortUp()">Price &uarr;</button>
                        <button @click="sortDown()">Price &darr;</button>
                    </div>
                    <div class="catalog__shop__cards">
                        <div class="card card-1" v-for="item in cards" :key="item.id">
                            <figure class="card__image-back">
                                <button class="card__image-btn">
                                    <p>
                                        <span>
                                            <svg
                                                width="27"
                                ...
                                <img :src="item.imgSource" :alt="item.imgAlt" class=" card__img">
                            </figure>
                            <div class="card__content">
                                <p class="card__small-title">{{item.smallTitle}}</p>
                                <p class="text card__text">{{item.text}}</p>
